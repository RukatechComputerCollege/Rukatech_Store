...
const Order = () => {
  ...
  const summary = useMemo(() => {
    ...
    const pending = orders.filter((order) =>
      ["received", "packaging", "on_the_road"].includes(order.orderStatus),
    ).length;
      ...
      <section className="rounded-lg sm:rounded-2xl md:rounded-[32px] border border-[#E5E7EB] bg-[#F8FDFF] p-4 sm:p-6 md:p-8 shadow-xl">
        <div className="flex flex-col gap-4 sm:gap-6 md:gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-2 sm:space-y-3">
            <p className="text-xs sm:text-sm uppercase tracking-[0.35em] text-[#0F766E]">
              Order management
            </p>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-[#111827]">
              Luxury order command center
            </h1>
            <p className="max-w-2xl text-xs sm:text-sm text-[#475569]">
              Monitor order performance, review fulfillment status, and access
              order details from a premium admin dashboard.
            </p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4 w-full lg:max-w-3xl">
            <div className="rounded-lg sm:rounded-2xl md:rounded-[24px] bg-white p-3 sm:p-4 md:p-6 shadow-sm border border-[#E5E7EB]">
              ...
              <p className="mt-2 sm:mt-3 md:mt-4 text-xl sm:text-2xl md:text-3xl font-bold text-[#111827]">
                {summary.totalOrders}
              </p>
            </div>
            <div className="rounded-lg sm:rounded-2xl md:rounded-[24px] bg-white p-3 sm:p-4 md:p-6 shadow-sm border border-[#E5E7EB]">
            ...
            <div className="rounded-lg sm:rounded-2xl md:rounded-[24px] bg-white p-3 sm:p-4 md:p-6 shadow-sm border border-[#E5E7EB]">
              ...
              <p className="mt-2 sm:mt-3 md:mt-4 text-xl sm:text-2xl md:text-xl font-bold text-[#111827]">
      ...
      <section className="rounded-lg sm:rounded-2xl md:rounded-[32px] border border-[#E5E7EB] bg-white p-4 sm:p-6 md:p-8 shadow-xl">
        <div className="flex flex-col gap-4 sm:gap-6 md:gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-[#475569]">
              Narrow results by order ID, customer email, or delivery status.
            </p>
          </div>
        ...
        <div className="flex flex-col gap-2 sm:gap-3 md:gap-4 sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 md:mb-8">
          <div>
            ...
            <p className="mt-1 text-xs sm:text-sm text-[#475569]">
              Showing {filteredOrders.length} of {summary.totalOrders} orders.
            </p>
          </div>
          <span className="inline-flex items-center rounded-full bg-[#ECFDF5] px-3 sm:px-4 py-1 sm:py-2 text-xs sm:text-sm font-semibold text-[#047857]">
            {filteredOrders.length} results
          </span>
        </div>

        {loading ? (
          <div className="rounded-lg sm:rounded-2xl md:rounded-[28px] border border-dashed border-[#CBD5E1] bg-[#F8FAFF] p-6 sm:p-8 md:p-12 text-center text-xs sm:text-sm text-[#475569]">
            Loading orders...
          </div>
          ...
          <div className="grid gap-3 sm:gap-4 md:gap-6">
            {filteredOrders.map((order) => (
              <div
                key={order._id}
                className="rounded-lg sm:rounded-2xl md:rounded-[28px] border border-[#E5E7EB] bg-[#F8FAFF] p-3 sm:p-4 md:p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"
              >
                <div className="flex flex-col gap-3 sm:gap-4 md:gap-6 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex flex-col gap-2 sm:gap-3 items-start sm:items-end w-full sm:w-auto">
                    <span
                      className={`inline-flex rounded-full px-3 sm:px-4 py-1 sm:py-2 text-xs font-semibold whitespace-nowrap ${statusBadge(order.orderStatus)}`}
                    >
                      {order.orderStatus.replace("_", " ").toUpperCase()}
                    </span>
                    <div className="text-right w-full sm:w-auto">
                      <p className="text-xs text-[#64748B]">Order total</p>
                      <p className="text-lg sm:text-2xl font-bold text-[#111827]">
                ...
                <div className="mt-4 sm:mt-6 grid gap-2 sm:gap-3 grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 text-xs sm:text-sm text-[#475569]">
                  <div className="rounded-lg sm:rounded-[20px] bg-white p-2 sm:p-3 md:p-4 border border-[#E5E7EB]">
                    ...
                    <p className="mt-1 sm:mt-2 text-base sm:text-lg font-semibold text-[#111827]">
                  ...
                  <div className="rounded-lg sm:rounded-[20px] bg-white p-2 sm:p-3 md:p-4 border border-[#E5E7EB]">
                    ...
                    <p className="mt-1 sm:mt-2 text-base sm:text-lg font-semibold text-[#111827] truncate">
                  ...
                  <div className="rounded-lg sm:rounded-[20px] bg-white p-2 sm:p-3 md:p-4 border border-[#E5E7EB]">
                    ...
                    <p className="mt-1 sm:mt-2 text-base sm:text-lg font-semibold text-[#111827] truncate">
                      {(order.userName || order.userEmail)?.substring(0, 10)}
                    </p>
                  </div>
                  <div className="rounded-lg sm:rounded-[20px] bg-white p-2 sm:p-3 md:p-4 border border-[#E5E7EB]">
                    <p className="text-xs uppercase tracking-[0.2em] text-[#94A3B8]">
