...
const OrderDetails = () => {
  ...
  const statusFlow = useMemo(
    () => ["received", "packaging", "on_the_road", "delivered", "cancelled"],
    [],
  );

  const hasOutOfStock = useMemo(() => {
    return order?.products?.some(
      (product) => !product.inStock || product.inventory <= 0,
    );
  }, [order?.products]);
              ...
              <div className="space-y-4">
                ...
                {order.orderStatus === "cancelled" && (
                  <div className="rounded-lg bg-[#FEE2E2] border border-[#FECACA] p-4 flex gap-3">
                    <FiAlertCircle
                      size={20}
                      className="text-[#B91C1C] flex-shrink-0 mt-0.5"
                    />
                    <div>
                      ...
                      <p className="text-xs text-[#7C2D12] mt-1">
                        This order has been cancelled and cannot be reactivated.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="rounded-[28px] border border-[#E5E7EB] bg-white p-6 shadow-sm">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-sm text-[#6B7280]">
                    {order.products?.length || 0} items in this order
                  </p>
                </div>
                <div className="rounded-full bg-[#F8FAFF] px-4 py-2 text-sm font-semibold text-[#111827]">
                  Summary
                </div>
              </div>
              <div className="mt-6 space-y-4">
                {order.products?.map((product, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="grid gap-4 md:grid-cols-[auto_1fr_auto] items-center rounded-[20px] bg-[#F8FAFF] p-4 relative"
                    >
                      {isOutOfStock && (
                      ...
                      <div
                        className={`h-20 w-20 rounded-[16px] overflow-hidden bg-[#E5E7EB] ${isOutOfStock ? "opacity-50" : ""}`}
                      >
                        {product.image ? (
                          <img
                            src={product.image}
                      ...
                      <div className={isOutOfStock ? "opacity-60" : ""}>
                        ...
                        <p className="text-sm text-[#6B7280]">
                          Unit: ₦{product.price?.toLocaleString() || "0"}
                        </p>
                      </div>
                      <p
                        className={`text-right text-lg font-bold ${isOutOfStock ? "text-[#94A3B8]" : "text-[#111827]"}`}
                      >
                        ₦
                        {(
            ...
            <div className="rounded-[28px] border border-[#E5E7EB] bg-white p-6 shadow-sm">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="text-[14px] font-semibold text-[#111827]">
                    {order.billingDetails?.email || order.userEmail || "N/A"}
                  </p>
                </div>
                <div>
                  <p className="text-[12px] text-[#5A607F] mb-1">Zip Code</p>
                  <p className="text-[14px] font-semibold text-[#111827]">
                    {order.billingDetails?.zipcode || "N/A"}
                  </p>
                </div>
              </div>
            </div>
