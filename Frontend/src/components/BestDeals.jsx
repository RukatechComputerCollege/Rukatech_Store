...
const BestDeals = () => {
  const [productWithHighestDiscount, setProductWithHighestDiscount] =
    useState(null);
  ...
  return (
    <div className="w-full gap-4 md:gap-0 grid items-start lg:grid-cols-[24%_76%] lg:border lg:border-[#E4E7E9]">
      <div
        className="border border-[#E4E7E9] md:border-0 md:border-r md:border-[#E4E7E9]"
        style={{ padding: "10px" }}
      >
        {/* for higher product */}
        {productWithHighestDiscount && (
          <div
            onClick={() => productDetails(productWithHighestDiscount)}
            className="w-full flex flex-col cursor-pointer"
          >
            {/* for highest discount product */}
            <div className="relative">
              <img
                src={productWithHighestDiscount?.image[0]}
                alt="product-image"
              />
              <span
                className="absolute top-0 left-0 rounded-xs bg-[#EFD33D]"
                style={{ padding: "5px 10px" }}
              >
                {productWithHighestDiscount.discountPercentage}% OFF
              </span>
              <span
                className="absolute top-10 left-0 rounded-xs bg-[#EE5858] text-white"
                style={{ padding: "5px 10px" }}
              >
                HOT
              </span>
            ...
            <div className="w-full flex flex-col gap-2">
              ...
              <p className="flex gap-2 items-center">
                <span className="text-[18px] text-[#ADB7BC] line-through">
                  ₦
                  {productWithHighestDiscount?.price
                    ? productWithHighestDiscount.price.toLocaleString()
                    : "0"}
                </span>
                <span className="text-[#2DA5F3] text-[18px] font-bold">
                  ₦
                  {productWithHighestDiscount?.discountprice
                    ? productWithHighestDiscount.discountprice.toLocaleString()
                    : "0"}
                </span>
              </p>
              <p className="text-[14px] text-[#5F6C72]">
                {productWithHighestDiscount.description
                  .split(" ")
                  .slice(0, 30)
                  .join(" ")}
                ...
              ...
              <div className="w-full grid grid-cols-[1fr_3fr_1fr] gap-2">
                <button
                  className="rounded-xs bg-[#FFE7D6] text-[#191C1F]"
                  style={{ padding: "12px" }}
                >
                  <FaRegHeart size={24} />
                </button>
                <button
                  ...
                  className="rounded-xs bg-[#FA8232] text-white flex items-center gap-2 cursor-pointer transition-all hover:bg-[#f7751f] active:bg-[#f89e62]"
                  style={{ padding: "12px" }}
                >
                  {isAddedToCart ? (
                    <FaMinus size={20} />
      ...
      <div className="w-full items-start md:items-stretch grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 grid-rows-auto gap-2 lg:gap-0">
        {bestDealsProduct && bestDealsProduct.length > 0
